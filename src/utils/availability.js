...
const getRangesWhenMoreThanOne = (availabilities, ranges) => {
  const first =
    availabilities[0].startHour === 8
      ? `${availabilities[0].endHour}-${availabilities[1].startHour}`
      : `8-${availabilities[0].startHour}`;
  ...
  while (idx <= availabilities.length - 1) {
    if (!availabilities[idx + 1]) {
      ...
      ranges.push(
        `${availabilities[idx].endHour}-${availabilities[idx + 1].startHour}`,
      );
    }
    idx++;
  }
